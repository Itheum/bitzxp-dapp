...
const GiverLeaderboard: React.FC<GiverLeaderboardProps> = (props) => {
  ...
  const [getterLeaderBoard, setGetterLeaderBoard] = useState<
    LeaderBoardItemType[]
  >([]);
    ...
    <div className="relative">
      ...
      >
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          className="flex flex-col max-w-[100%] p-[.5rem] mb-[3rem] rounded-[1rem]"
        >
          ...
          {giverLeaderBoardIsLoading ? (
            ...
            <div className="flex">
              {getterLeaderBoard && getterLeaderBoard.length > 0 ? (
                <LeaderBoardTable
                  leaderBoardData={getterLeaderBoard}
                  address={address}
                  showMyPosition={showUserPosition}
                />
              ) : (
