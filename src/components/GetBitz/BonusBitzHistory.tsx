...
export const explorerAddress = 'https://explorer.solana.com';

const BonusBitzHistory: React.FC<LeaderBoardTableProps> = (props) => {
  const { leaderBoardData, address, showMyPosition = false } = props;
  const myPosition = showMyPosition
    ? leaderBoardData.findIndex((item) => item.playerAddr === address)
    : -1;
    ...
    <div className="flex flex-col justify-center items-center w-full">
      {showMyPosition && (
        <span className="text-xs text-center mb-2">
          Your position * {myPosition >= 0 ? myPosition + 1 : '20+'} *
        </span>
      )}
